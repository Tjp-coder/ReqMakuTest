package v1.testcase.role;

import v1.api.Org;
import v1.api.Role;
import v1.api.User;
import v1.base.BaseTest;
...
public class RoleUserTest extends BaseTest {
    ...
    void setUp() {
        //添加一条机构
        orgSaveData = JsonPathUtils.jsonPathParseByFile("/v1/template/org/save.json");
        orgSaveData.set("$.id",1);
        new Org().save(token,orgSaveData.json());

        //添加1条角色数据，角色属于上一条机构里
        roleSaveData = JsonPathUtils.jsonPathParseByFile("/v1/template/role/save.json");
        roleSaveData.set("$.id", 2);
        new Role().save(token,roleSaveData.json());

        //添加一条用户数据
        userSaveData = JsonPathUtils.jsonPathParseByFile("/v1/template/user/save.json");
        userSaveData.set("$.id",3);
        new User().save(token,userSaveData.json());

        orgSaveData = JsonPathUtils.jsonPathParseByFile("/v1/template/org/save.json");
        roleSaveData = JsonPathUtils.jsonPathParseByFile("/v1/template/role/save.json");
        userPageData = JsonPathUtils.jsonPathParseByFile("/v1/template/role/userPageData.json");
        userSaveData = JsonPathUtils.jsonPathParseByFile("/v1/template/user/save.json");
