...
public class PutPassword extends BaseTest {
    ...
    void setUp() {
        //准备前置数据
        //准备一条id为1的机构数据
        defaultData = JsonPathUtils.jsonPathParseByFile("/v1/template/org/save.json");
        new Org().save(token, defaultData.set("$.id", orgId).json());

        //添加5个用户账号
        defaultData = JsonPathUtils.jsonPathParseByFile("/v1/template/user/save.json");
        for (int i = 1; i <= 5; i++) {
    ...
    void readDefault() {
        id++;
        integers.add(id);
        defaultData = JsonPathUtils.jsonPathParseByFile("/v1/template/user/password.json");
    ...
    void delete() {
        ...
        user.delete(token, integers)
                .then().statusCode(200);
        defaultData = JsonPathUtils.jsonPathParseByFile("/v1/template/auth/login.json");
